...
(0, ($sideLen - 1), ($data.Count -1 - ($sideLen - 1)), ($data.Count - 1)) | %{$data[$_]=1}

if (-not $real) {
    Write-host "Initial Board (with corners on)"
    printBoard $data $sideLen
}
...
foreach ($cycle in (1..$cycles)) {
    $data = for ($i = 0; $i -lt $data.Count; $i++) {
        ...
        if ($i -in (0, ($sideLen - 1), ($data.Count -1 - ($sideLen - 1)), ($data.Count - 1))) {
            1
        }
        else {
