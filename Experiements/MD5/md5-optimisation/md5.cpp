...
template<typename HT, void(&fn)(MD5_STATE<HT>*, const void*)>
void md5_testing(MD5_STATE<HT>* state, const void* __restrict__ src, size_t len, int cycle) {
	md5_init<HT>(state);
	char* __restrict__ _src = (char* __restrict__)src;
	uint64_t totalLen = len << 3; // length in bits
	for(; len >= 64; len -= 64) {
...
int main(int argc, char *argv[]) {
	...
	for (size_t i = 0; i < (size_t)std::stoi(argv[2]); i++)
	{
		// std::cout<<i<<std::endl;
		// printf("sizeof(argv[1]) = %i\n",input.length());
		md5<uint32_t, md5_block_nolea>(&hash, input.c_str(), input.length());
		std::ostringstream  s;
		s << std::setw(8) << std::setfill('0') << std::hex << _byteswap_ulong(hash.A);
		s << std::setw(8) << std::setfill('0') << std::hex << _byteswap_ulong(hash.B);
		s << std::setw(8) << std::setfill('0') << std::hex << _byteswap_ulong(hash.C);
		s << std::setw(8) << std::setfill('0') << std::hex << _byteswap_ulong(hash.D);
		input = s.str();
	}

	// md5_testing<uint32_t, md5_block_nolea>(&hash, input.c_str(), input.length(),std::stoi(argv[2]));
	// 	std::ostringstream  s;
	// 	s << std::setw(8) << std::setfill('0') << std::hex << _byteswap_ulong(hash.A);
	// 	s << std::setw(8) << std::setfill('0') << std::hex << _byteswap_ulong(hash.B);
	// 	s << std::setw(8) << std::setfill('0') << std::hex << _byteswap_ulong(hash.C);
