...
function Solution {
    ...
    while ($subCircuits[0].Count -ne $data.Count) {
        $distance = $sortedDistances[$distanceIndex]
    ...
    $lastPair = $distances[$sortedDistances[$distanceIndex - 1]]
    $lastPair.j1.x * $lastPair.j2.x

}
Unit-Test  ${function:Solution} (@{Path = "$PSScriptRoot/testcases/test1.txt"; Joins = -1 }) 25272
$measuredTime = measure-command { $result = Solution (@{Path = "$PSScriptRoot\input.txt"; Joins = -1 }) }
