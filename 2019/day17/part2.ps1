...
function Solution {
    ...
    $A = "L,8,R,12,R,12,R,10`n".ToCharArray()
    $B = "R,10,R,12,R,10`n".ToCharArray()
    $C = "L,10,R,10,L,6`n".ToCharArray()

    $main = "A,B,A,B,C,C,B,A,B,C`n".ToCharArray()
    ...
    foreach($seq in $main,$a,$b,$c){
        foreach($char in $seq){
            $ASCII.RunComputer([int]$char)
            $ASCII.outputBuffer | % { [char]$_ } | Join-String -Separator "" | write-host
            $ASCII.outputBuffer = @()
        }
    }
    $ASCII.RunComputer([int][char]"n")
    $ASCII.RunComputer([int][char]"`n")
    # $ASCII.outputBuffer | % { [char]$_ } | Join-String -Separator "" | write-host
