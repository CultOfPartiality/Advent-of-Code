...
Import-Module -Name "$PSScriptRoot\..\..\OtherUsefulStuff\CoordsClass\Class_Coords\bin\Debug\net9.0\Coord2D.dll"
...
function Solution {
	...
	for ($x = 0; $x -lt $width; $x++) {
		for ($y = 0; $y -lt $height; $y++) {
			if ($data[$y][$x] -eq "#") {
				$asteroid = [Coord2D.Coord]::New($y, $x)
				$asteroids[$asteroid.Hash($width)] = $asteroid
			}
		}
	}
	function PolarAngle([Coord2D.Coord]$dir) {
	...
	for ($x = 0; $x -lt $width; $x++) {
		for ($y = 0; $y -lt $height; $y++) {
			...
			$gcd = gcd $x $y
			$direction = [Coord2D.Coord]::new($y / $gcd, $x / $gcd)
	...
	$delta = [Coord2D.Coord]::New(0, 0) # Cloning is very slow
