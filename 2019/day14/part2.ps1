...
function Solution {
   param ($Path)
    ...
    while($NeedToMake.Count){
        ...
        # For part 2, we just perform all calcs using floats, and round down at the end
        $ActualBatchesToMake = $Make.AmountRequired/$Make.Recipie.Amount
        $ActualAmountToMake = $Make.AmountRequired/$Make.Recipie.Amount*$Make.Recipie.Amount
    ...
    # For part 2, we just perform all calcs using floats, and round down at the end
    [Math]::Floor(1000000000000/$totalOre)
    
}
Unit-Test  ${function:Solution} "$PSScriptRoot/testcases/test3.txt" 82892753
Unit-Test  ${function:Solution} "$PSScriptRoot/testcases/test4.txt" 5586022
Unit-Test  ${function:Solution} "$PSScriptRoot/testcases/test5.txt" 460664
