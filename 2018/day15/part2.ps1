...
function Solution {
    ...
    $elfAttackLimits = 3,100
    $elfDied = $true
    $elfAttackPower = 3
    
    while ($elfAttackLimits[1] -ne $elfAttackLimits[0]) {

        # Binary search our way to the ideal attack power
        if($elfDied){
            $elfAttackLimits[0] = $elfAttackPower+1
            write-host "Elf died (or round 0). New Range=($elfAttackLimits)"
        }
        else{
            $elfAttackLimits[1] = $elfAttackPower
            write-host "Elves won. New Range=($elfAttackLimits)"

        }
        
        $elfDied = $false
        #set the attack power in the middle
        $elfAttackPower = [math]::Floor(($elfAttackLimits[1]-$elfAttackLimits[0])/2 + $elfAttackLimits[0])

        write-host "Attack power: $elfAttackPower"
        $map = @{}
        $walls = @{}
        $combatants = @()
        $rows = $rawdata.Length
            ...
            for ($x = 0; $x -lt $cols; $x++) {
                ...
                if ($cell -in "G", "E") {
                    $combatants += [PSCustomObject]@{
                        ...
                        Coord  = $coord
                        Health = 200
                        Attack = ($cell -eq "G" ? 3 : $elfAttackPower)
                    }
        ...
        :sim while ( ($combatants.Team | group).Name.Count -eq 2) {
            ...
            foreach ($combatant in $combatants) {
                ...
                if ($enemysNextTo.Count -gt 0) {
                    ...
                    $enemyToAttack.Health -= $combatant.Attack
...
Unit-Test  ${function:Solution} "$PSScriptRoot/testcases/test1.txt" 4988 #15 attack
Unit-Test  ${function:Solution} "$PSScriptRoot/testcases/test4.txt" 31284 #4 attack
Unit-Test  ${function:Solution} "$PSScriptRoot/testcases/test5.txt" 1140 #34 attack
