...
function Solution {
    ...
    :inf while ($true) {
        ...
        foreach ($cart in $carts) {
            ...
            # Check for collisions, and remove both carts
            if (($carts.coords | ? { $_ -eq $cart.coords } ).Count -gt 1) {
                $carts = $carts | ?{$_.coords -ne $cart.coords}
            }
    
        }
        #check if we've only got one cart
        if($carts.Count -eq 1){
            break inf
        }
    }
...
Unit-Test  ${function:Solution} "$PSScriptRoot/testcases/test2.txt" "6,4"
